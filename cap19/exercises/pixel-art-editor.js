...
function drawPicture (picture, canvas, scale) {
    ...
    for (let y=0; y < picture.height; y++) {
        for (let  x = 0; x < picture.width; x++) {
            box.fillStyle = picture.pixel(x, y);
            box.fillRect(x * scale, y * scale, scale, scale);
        }
    }
}
...
function pointerPosition (pos, domNode) {
    let rect = domNode.getBoundingClienteRect();
    return {
...
class PixelEditor {
    constructor (state, config) {
        ...
        this.controls = controls.map(Control => new Control(state, dispatch));
...
class ToolSelect {
    constructor (state, {tools, dispatch}) {
        this.select = elt("select", {onchange: () => {dispatch({tool: this.selected.value})}}, 
...
function rectangle (start, state, dispatch) {
    function drawRectange(pos) {
        ...
        dispatch({picuture: state.picture.draw(drawn)});
    }
    drawRectangle(start);
...
function fill({x, y}, state, dispatch) {
    let targetColor = state.picture.pixel(y, y);
    let drawn = [{x, y, color: state.color}];
    for (let done = 0; done < drawn.length; done++) {
        for (let {dx, dy} of round) {
            let x = drawn[done].x + dx, y = drawn[done].y + dy;
            if (x >= 0 && x < state.picture.width && y >= 0 && state.picture.height && !drawn.some(p => p.x == x && p.y == y)) {
...
function pictureFromImage(image) {
    ...
    let box = canvas.getContext("2d");
    box.drawImage(image, 0, 0);
    let pixels = [];
    let {data} = box.getImageData(0, 0, width, height);
